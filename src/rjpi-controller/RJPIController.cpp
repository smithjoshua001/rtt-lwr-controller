...
RJPIController::RJPIController(std::string const & name) :
	...
	this->addOperation("loadURDFAndSRDF", &RJPIController::loadURDFAndSRDF, this,
			RTT::ClientThread);
...
void RJPIController::updateHook() {
	...
	robot_state.angles.tail<7>().reverseInPlace();
	robot_state.velocities.tail<7>().reverseInPlace();
